...
namespace
{
    ...
    void getPriWindowSettings(voip2::WindowSettings& ws, const float scale)
    {
        ...
        ws.conference.aspectRatio = 16.f/9.f;// 4.f / 3;
        ...
        ws.conference.forcePrimaryVideoCrop = true;
...
namespace voip_manager
{
    ...
    private:
        ...
        void _load_avatars       (const std::string& account_uid, const UserID& user_uid, bool request_if_not_exists = true);
    ...
    VoipManagerImpl::VoipManagerImpl(core::core_dispatcher& dispatcher)
        ...
        , _voipMuteFix(false)
        , _async_tasks(std::make_shared<core::async_executer>())
    ...
    void VoipManagerImpl::_update_peer_list(CallDescPtr call, const std::string& account_uid, const UserID& user_uid)
    {
        ...
        if (cp.user_uids)
        {
            ...
            while (true)
            {
                ...
                ConnectionDescPtr newConnection;
                if (!_connection_get(normalizeUid(account_uid), uid, newConnection))
                {
                    newConnection = _create_connection(normalizeUid(account_uid), uid);
                }
    ...
    void VoipManagerImpl::_update_layout(CallDescPtr callDesc)
    {
        ...
        std::for_each(_windows.begin(), _windows.end(), [active_video_conf, this, callDesc, engine, outgoing](WindowDesc& window_desc)
        {
                if (window_desc.talkKey == callDesc->callKey)
                {
                    ...
                    else if (active_video_conf)
                    {
                        ...
                        engine->WindowShowButton(voip2::ButtonType_GoToChat, true);
                    }
                    else
                    {
                        lt = LayoutType_One;
        ...
        if (active_video_conf)
        {
            if (_windows.size() > 0)
            {
                window_add_button(voip2::ButtonType_Close, voip2::ButtonPosition_TopRight, 0, 0);
                window_add_button(voip2::ButtonType_GoToChat, voip2::ButtonPosition_BottomRight, - 8.0f * _windows[0].scaleCoeffitient, - 8.0f * _windows[0].scaleCoeffitient);
            }
        }
    }
    ...
    void VoipManagerImpl::media_video_en(bool enable)
    {
        ...
        // We enable video device is selected correct video camture type.
    ...
    void VoipManagerImpl::_window_set_av(const std::string& contact, voip2::AvatarType type, const UserBitmap& bmp) {
        ...
        if (voip2::AvatarType_Camera == type || voip2::AvatarType_CameraCrossed == type || voip2::AvatarType_Foreground == type) {
            engine->WindowSetAvatar2(PREVIEW_RENDER_NAME, bmp.bitmap.data, bmp.bitmap.w, bmp.bitmap.h, type);
        } else
        {
    ...
    void VoipManagerImpl::_load_avatars(const std::string& account_uid, const UserID& user_uid, bool request_if_not_exists/* = true*/) {
    ...
    void VoipManagerImpl::_set_device(voip2::DeviceType device_type, const std::string& device_guid, bool innerSet)
    {
        VOIP_ASSERT_RETURN(voip2::MAX_DEVICE_GUID_LEN >= device_guid.length());
        {
            ...
            case voip2::VideoCapturing:
            {
                ...
                if (!!engine)
                {
                    engine->GetVideoDeviceType(device_guid.c_str(), videoDeviceType);
                }
                _hasCamera = _hasCamera || (videoDeviceType == voip::VC_DeviceCamera);
                _hasScreenSharing = _hasScreenSharing || (videoDeviceType == voip::VC_DeviceDesktop);
    ...
    bool VoipManagerImpl::_setDevice(std::shared_ptr<voip2::Voip2> engine, voip2::DeviceType deviceType, const char *deviceUid, bool innerSet)
    {
        if (!!engine)
        {
            ...
            SIGNAL_NOTIFICATOION(kNotificationType_MediaLocVideoDeviceChanged, &desc);
        }
