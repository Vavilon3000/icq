...
namespace core
{
    namespace themes
    {
        ...
        :	cache_(std::make_shared<cache>(_themes_path)),
        ...
        std::shared_ptr<result_handler<bool, const download_task&>> face::get_next_meta_task()
        {
            ...
            auto themes_cache = cache_;
            auto task = std::make_shared<download_task>("", L"", 0);

            thread_->run_async_function([themes_cache, task]()->int32_t
                                        {
                                            if (!themes_cache->get_next_meta_task(*task))
                                                return -1;
        ...
        void face::clear_missing()
        {
            cache_->clear_missing();
        }
        ...
        {
            auto iter_bgc = _node.FindMember("BackgroundColor1");
            if (iter_bgc != _node.MemberEnd() && iter_bgc->value.IsString())
                bg_color_ = rapidjson_get_string(iter_bgc->value);

            auto iter_time_color = _node.FindMember("TimeColor");
            if (iter_time_color != _node.MemberEnd() && iter_time_color->value.IsString())
                time_color_ = rapidjson_get_string(iter_time_color->value);
        }
        ...
        void theme::typing::unserialize(const rapidjson::Value &_node)
        {
            ...
            if (iter_light_gif != _node.MemberEnd() && iter_light_gif->value.IsString())
                light_gif_ = std::stoi(rapidjson_get_string(iter_light_gif->value), 0);
        }
        ...
        bool theme::unserialize(const rapidjson::Value& _node, const ThemesScale _themes_scale)
        {
            ...
            if (iter_id != _node.MemberEnd() && iter_id->value.IsString())
                theme_id_ = std::stoi(rapidjson_get_string(iter_id->value));
            ...
            if (iter_image != _node.MemberEnd() && iter_image->value.IsObject())
            {
                ...
                {
                    image_name_ = rapidjson_get_string(iter_image_name->value);
                }
            }
            ...
            auto iter_delivery = _node.FindMember("DeliveryStatus");
            if (iter_delivery != _node.MemberEnd() && iter_delivery->value.IsObject())
            {
                auto iter_light = iter_delivery->value.FindMember("Light");
                if (iter_light != iter_delivery->value.MemberEnd() && iter_light->value.IsString())
                    delivery_status_light_ = (rapidjson_get_string(iter_light->value) == "1");
            }
            return true;
        }
        ...
        bool theme::get_delivery_status_light() const
        {
            return delivery_status_light_;
        }
        /* download_task */
        download_task::download_task(const std::string& _source_url, const std::wstring& _dest_file, time_t _modified_time, int32_t _theme_id)
        ...
        time_t download_task::get_last_modified_time() const
        {
        ...
        bool cache::parse(core::tools::binary_stream& _data, bool _insitu)
        {
            ...
            if (iter_base_url != doc.MemberEnd() && iter_base_url->value.IsString())
                base_url_ = rapidjson_get_string(iter_base_url->value);
        ...
        time_t cache::get_theme_thumb_time(const theme& _theme) const
        {
            const fs::path path(get_theme_thumb_path(_theme.get_theme_id()));

            boost::system::error_code e;
            if (fs::exists(path, e))
                return fs::last_write_time(path, e);
            return 0;
        }

        time_t cache::get_theme_image_time(const theme& _theme) const
        {
            const fs::path path(get_theme_image_path(_theme.get_theme_id()));
        ...
        void cache::make_download_tasks()
        {
            ...
            {
                const auto thumb_url = get_theme_thumb_url(*theme);
                const auto thumb_file_name = theme->get_thumb_path();
                const auto thumb_time = get_theme_thumb_time(*theme);
                meta_tasks_.push_back(download_task(thumb_url, thumb_file_name, thumb_time));

                const auto image_url = get_theme_image_url(*theme);
                const auto image_file_name = theme->get_image_path();
                const auto image_time = get_theme_image_time(*theme);
                themes_tasks_.push_back(download_task(image_url, image_file_name, thumb_time));
            }
        }
        ...
        void cache::make_download_task(theme& _theme)
        {
            ...
            const auto thumb_url = get_theme_thumb_url(_theme);
            const auto thumb_file_name = _theme.get_thumb_path();
            const auto thumb_time = get_theme_thumb_time(_theme);
            auto meta_task = download_task(thumb_url, thumb_file_name, thumb_time, theme_id);
            meta_task.push_request(0);
            meta_tasks_.push_back(meta_task);

            const auto image_url = get_theme_image_url(_theme);
            const auto image_file_name = _theme.get_image_path();
            const auto image_time = get_theme_image_time(_theme);
            auto theme_task = download_task(image_url, image_file_name, image_time, theme_id);
        ...
        void cache::serialize_meta_sync(coll_helper _coll)
        {
            ...
            for (auto theme: themes_list_)
            {
                ...
                coll_theme.set_value_as_int("id", theme->get_theme_id());
                coll_theme.set_value_as_int("position", theme->get_position());
                coll_theme.set_value_as_bool("tile", theme->is_tile());
                coll_theme.set_value_as_string("tint_color", theme->get_tint_color());
                coll_theme.set_value_as_string("typing_color", theme->get_typing_color());
                coll_theme.set_value_as_bool("delivery_status_light", theme->get_delivery_status_light());

                std::wstring file_name = theme->get_thumb_path();
                ...
                date_set.set_value_as_string("bg_color", theme->date_.bg_color_);
                date_set.set_value_as_string("text_color", theme->date_.text_color_);
                coll_theme.set_value_as_collection("date", date_set.get());

                coll_helper incoming_bubble_set(_coll->create_collection(), true);
                incoming_bubble_set.set_value_as_string("bg_color",   theme->incoming_bubble_.bg_color_);
                incoming_bubble_set.set_value_as_string("time_color", theme->incoming_bubble_.time_color_);
                ...
                outgoing_bubble_set.set_value_as_string("bg_color",   theme->outgoing_bubble_.bg_color_);
                outgoing_bubble_set.set_value_as_string("time_color", theme->outgoing_bubble_.time_color_);
                ...
                preview_stickers_set.set_value_as_string("time_color", theme->preview_stickers_.time_color_);
                ...
                chat_event_set.set_value_as_string("bg_color",   theme->chat_event_.bg_color_);
                chat_event_set.set_value_as_string("text_color", theme->chat_event_.text_color_);
                ...
                contact_name_set.set_value_as_string("text_color", theme->contact_name_.text_color_);
                ...
                new_messages_plate_set.set_value_as_string("bg_color",   theme->new_messages_plate_.bg_color_);
                new_messages_plate_set.set_value_as_string("text_color", theme->new_messages_plate_.text_color_);
                ...
                typing_set.set_value_as_int("light_gif",     theme->typing_.light_gif_);
                typing_set.set_value_as_string("text_color", theme->typing_.text_color_);
        ...
        {
            boost::system::error_code e;
            fs::remove_all(themes_path_, e);
        }

        void cache::clear_missing()
        {
            boost::system::error_code e;

            fs::path themes_root(themes_path_);
            ...
            for (fs::directory_iterator dir_it(themes_root, e); dir_it != end; ++dir_it)
            {
                fs::path path = (*dir_it);
                const auto dir_name = path.leaf().wstring();

                if (dir_name != themes_meta_file_name_)
                {
                    auto id = 0;
                    try
                    {
                        id = std::stoi(dir_name);
                    }
                    catch (...)
                    {
                        continue;
                    ...
                    auto it = find_if(themes_list_.begin(), themes_list_.end(), [id](auto theme) { return theme->get_theme_id() == id; });
                    if (it == themes_list_.end())
