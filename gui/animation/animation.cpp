...
namespace
{
    constexpr int animationTimeout()
    {
        return 16;
    }
}
...
namespace anim
{
    transition linear = [](double delta, double dt) {
        return delta * dt;
    };

    transition sineInOut = [](double delta, double dt) {
        return -(delta / 2.0) * (std::cos(M_PI * dt) - 1.0);
    };

    transition halfSine = [](double delta, double dt) {
        return delta * std::sin(M_PI * dt / 2);
    };

    transition easeOutBack = [](double delta, double dt) {
    ...
    transition easeInCirc = [](double delta, double dt) {
        return -delta * (std::sqrt(1.0 - dt * dt) - 1.0);
    };

    transition easeOutCirc = [](double delta, double dt) {
        const double t = dt - 1.0;
        return delta * std::sqrt(1.0 - t * t);
    };

    transition easeInCubic = [](double delta, double dt) {
        return delta * dt * dt * dt;
    };

    transition easeOutCubic = [](double delta, double dt) {
        const double t = dt - 1.0;
        return delta * (t * t * t + 1.0);
    };

    transition easeInQuint = [](double delta, double dt) {
        const double t2 = dt * dt;
        return delta * t2 * t2 * dt;
    };

    transition easeOutQuint = [](double delta, double dt) {
        const double t = dt - 1.0;
        const double t2 = t * t;
