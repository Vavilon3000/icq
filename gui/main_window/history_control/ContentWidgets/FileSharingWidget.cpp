...
namespace HistoryControl
{
    ...
    QRect FileSharingWidget::getLastPreviewGeometry()
    {
        if (isImagePreview())
            return PreviewContentWidget::getLastPreviewGeometry();
        return QRect();
    }
    ...
    void FileSharingWidget::onVisibilityChanged(const bool isVisible)
    {
        ...
        if (ShareButton_)
            ShareButton_->onVisibilityChanged(isVisible);
    ...
    void FileSharingWidget::saveAs()
    {
        ...
        Utils::saveAs(Metainfo_.Filename_, [this, wr_ref](const QString& file, const QString& dir)
        {
            ...
            const auto addTrailingSlash = (!dir.endsWith(ql1c('\\')) && !dir.endsWith(ql1c('/')));
            const auto slash = ql1s(addTrailingSlash ? "/" : "");
            const QString fullname = dir % slash % file;
    ...
    void FileSharingWidget::convertToUploadErrorView()
    {
        ...
        Metainfo_.Filename_ = qsl("not found");
        Metainfo_.FileSize_ = 0;
    ...
    {
        PreviewContentWidget::mouseMoveEvent(event);
    ...
    void FileSharingWidget::mouseReleaseEvent(QMouseEvent *event)
    {
        ...
        {
            const auto openPreviewer = (fullImageReady && !isGif());
            if (openPreviewer)
            {
    ...
    {
        return isState(State::PlainFile_Downloaded) || isState(State::PlainFile_Uploaded);
    }
    ...
    bool FileSharingWidget::loadPreviewFromLocalFile()
    {
        ...
        {
            Ui::MediaContainer* container = Ui::getMediaPreview(FsInfo_->GetLocalPath());

            QObject::connect(container, &Ui::MediaContainer::frameReady, this, [this](const QImage& _frame, const QString& _file)
            {
                if (_file != FsInfo_->GetLocalPath())
                    return;

                localPreviewLoaded(QPixmap::fromImage(_frame));
            });
        }
    ...
    void FileSharingWidget::openDownloadsDir() const
    {
        ...
        const QString param = ql1s("/select,") + QDir::toNativeSeparators(DownloadedFileLocalPath_);
    ...
    void FileSharingWidget::renderDataTransferProgress(QPainter &p)
    {
        ...
        const auto baseAngle = (animation_.current() * 16);
    ...
    void FileSharingWidget::renderDataTransferProgressText(QPainter &p)
    {
        ...
        const QString text = formatFileSize(BytesTransferred_) % ql1s(" of ") % Metainfo_.FileSizeStr_;
    ...
    void FileSharingWidget::requestPreview()
    {
        ...
        PreviewDownloadId_ = Ui::GetDispatcher()->downloadImage(previewUri, qsl("#"), QString(), false, 0, 0, false);
    }
    ...
    void FileSharingWidget::setInitialWidgetSizeAndState()
    {
        ...
        QFileInfo fileInfo(FsInfo_->GetLocalPath());
        const auto ext = fileInfo.suffix();
        if (ext.isEmpty() || (!Utils::is_image_extension(ext) && !Utils::is_video_extension(ext)))
        {
            setState(State::PlainFile_Initial);
            return;
        }
    ...
    void FileSharingWidget::imageDownloaded(qint64 seq, QString uri, QPixmap preview, QString)
    {
        ...
        Preview_.FullImg_ = std::move(preview);
        ...
        QString seq, bool success, QString localPath, QString uri, int contentType, bool isFileTooBig)
    {
        ...
        if (isState(State::ImageFile_Uploading) || isState(State::ImageFile_Initial))
        {
    ...
    void FileSharingWidget::localPreviewLoaded(QPixmap pixmap)
    {
        ...
        Preview_.FullImg_ = std::move(pixmap);

        setPreview(Preview_.FullImg_);

        if (!isState(State::ImageFile_Uploaded))
