...
const QString TOOLTIP_STYLE = qsl(
...
{
    // ToolTip position is above of the parent window.
...
void ToolTipLabel::paintEvent(QPaintEvent* _e)
{
    ...
    if (parentWidget())
    {
        ...
        painter.strokePath(path, QPen(CommonStyle::getColor(CommonStyle::Color::GRAY_BORDER), Utils::scale_value(2)));
    }
}